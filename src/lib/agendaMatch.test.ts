...
const escolas = new Map([["e1", "EMEIEF Atiorô"]]);
const estudantes = [
  ent({ id: "s1", full_name: "RENNAN DANTAS FARIAS", school_id: "e1", grade: "7º ano" }),
  ent({ id: "s2", full_name: "João Gabriel da Silva", school_id: "e1", grade: "8º ano" }),
];
const pacientes = [ent({ id: "p1", full_name: "Paciente Exemplo A" })];

describe("normalização de nomes", () => {
  it("ignora acento, caixa e pontuação", () => {
    expect(normalizarNome("RENNAN DANTAS FARIAS")).toBe("rennan dantas farias");
...
describe("classificação", () => {
  ...
  it("correspondência exata é segura e já vem escolhida", () => {
    const r = classificar([linha("Rennan Dantas Farias")], estudantes, pacientes, escolas);
    expect(r[0].classificacao).toBe("segura");
    expect(r[0].escolhido).toBe("s1");
    expect(r[0].candidatos[0].detalhe).toContain("Atiorô");
  });

  it("nome parcial vira sugestão que exige confirmação", () => {
    const r = classificar([linha("João Gabriel")], estudantes, pacientes, escolas);
  ...
  it("resumo conta cada classificação", () => {
    const r = classificar(
      [linha("Rennan Dantas Farias"), linha("João Gabriel"), linha("Ninguém")],
      estudantes,
      pacientes,
      escolas,
    );
...
describe("duplicados", () => {
  const existentes = [
    { event_date: "2026-06-23", start_at: "08:30", student_id: "s1", title: "Rennan" },
  ];

  it("detecta mesmo horário, data e pessoa", () => {
    expect(ehDuplicado(existentes, "2026-06-23", "08:30", "s1", "Rennan")).toBe(true);
  });

  it("horário ou data diferentes não é duplicado", () => {
    expect(ehDuplicado(existentes, "2026-06-23", "09:30", "s1", "Rennan")).toBe(false);
    expect(ehDuplicado(existentes, "2026-06-24", "08:30", "s1", "Rennan")).toBe(false);
  });

  it("sem vínculo, compara pelo nome", () => {
    expect(ehDuplicado(existentes, "2026-06-23", "08:30", "", "RENNAN")).toBe(true);
  });
});
