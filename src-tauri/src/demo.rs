...
mod tests {
    ...
    fn seed_and_clear() {
        ...
        seed(&c, &key).unwrap();
        let n: i64 = c
            .query_row("SELECT COUNT(*) FROM patients WHERE is_demo = 1", [], |r| {
                r.get(0)
            })
            .unwrap();
        assert_eq!(n, 1);
